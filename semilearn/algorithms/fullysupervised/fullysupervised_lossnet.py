...
from semilearn.algorithms.utils import ce_loss, mixup_one_target,get_estimate_targets
...
class FullySupervised_lossnet(AlgorithmBase):
    """
    ...
    def train_step(self, idx_lb,x_lb, y_lb,y_true):
        ...
        with self.amp_cm():
            ...
            if self.args.meta_goal is not None:
                ...
                self.meta_loader = iter(self.loader_dict['meta'])
                data_meta = next(self.meta_loader)                
                ...
                # e_targets = get_estimate_targets(idx_lb,logits_x_lb,self.etargets,t_beta=0.9)
            # print(w_norm)
            # self.args.meta_goal = None if self.args.meta_goal == 'None' else self.args.meta_goal
            # if self.args.meta_goal is not None:

                sup_loss_w = torch.mean(ce_loss(logits_x_lb, y_lb, reduction='none')*torch.squeeze(w))
                ...
                sup_loss_true_weighted=0
                sup_loss_raw = ce_loss(logits_x_lb, y_lb, reduction='mean').item()
                for key, value in weight_log.items():
                    self.log[key].append(value)
                # loss_es = ce_loss(logits_x_lb,e_targets, reduction='mean')
                # sup_loss = sup_loss_w+loss_es
                sup_loss = sup_loss_w
            else:
        ...
        tb_dict['train/sup_loss_weighted'] = sup_loss_w.item()
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            for data_lb in self.loader_dict['train_lb']:
                ...
                if self.algorithm=='fullysupervised_lossnet': 
                # and not self.args.meta_goal.endswith('_expno1N'):
                    if self.args.net=='wrn_28_10' and not self.args.meta_goal.endswith('_expno1N'):    
                        lr = self.args.lr * ((0.1 ** int(self.it >= 18000)) * (0.1 ** int(self.it >= 19000))) 
                            
                        for param_group in self.optimizer.param_groups:
                            param_group['lr'] = lr
                                    
                    elif self.args.net=='resnet34':
                        # if self.it==15640 or self.it==31280: #wrn_28_10
                        lr = self.args.lr * ((0.1 ** int(self.it >= 15640)) * (0.1 ** int(self.it>= 31280))) 
                        # print('-'*100,epoch)
                        for param_group in self.optimizer.param_groups:
                            param_group['lr'] = lr 
                    elif self.args.net=='resnet32':
                        
                        lr = self.args.lr * ((0.1 ** int(self.it >= 20000)) * (0.1 ** int(self.it>= 25000))) 
                        # print('-'*100,epoch)
                        for param_group in self.optimizer.param_groups:
                            param_group['lr'] = lr                             
