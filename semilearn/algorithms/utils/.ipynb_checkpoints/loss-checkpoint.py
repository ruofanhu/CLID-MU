...
# def entropy(pk_batch,base=None):
#     "pk_batch is the logits for one batch "
#     pk_batch = F.softmax(pk_batch,dim=-1)
#     pk_batch = pk_batch / pk_batch.sum(dim=1)[:, None]
# #     print(pk_batch*torch.log(pk_batch))
#     s = -torch.sum(pk_batch*torch.log(pk_batch),axis=1)
#     if base is not None:
#         s /= torch.log(torch.Tensor([base]))
#     return s    
...
#     # drop the instances too close to the points
#     epsilion = 1e-12
#     selected=selected[selected>epsilion]
#     #######
# #     selected[selected==0]+=1e-12
#     # selected += 1e-12
#     lids = -k/torch.sum(torch.log(selected/(selected[:,-1]).reshape(-1,1)),axis=1)
...
#     y_one_= y_one+eps
#     sumf=y_one_.sum()
#     t=sumf/y_one_.sum(dim=0)
#     w_c=t/t.sum()
#     print(w_c,'\n',(w_c*y_one)[y_one==1].sum())
#     w=(w_c*y_one)[y_one==1]/(w_c*y_one)[y_one==1].sum()
#     print(w)
#     return w
...
# kernel: apply kernel trick or not
    ...
    out= torch.sum(w*entr)
    return out
# def entropy_intra(x, y = None):

#     if y is None:
#         y = torch.zeros(len(x)).cuda()

#     entr = torch.zeros(2).cuda()
...
def pc_loss(feat, logits,class_prototype,temperature):
    ## https://github.com/salesforce/RRL/blob/main/algorithms/Model.py
    # calculate similarity to corresponding class prototype
...
# https://github.com/lijin118/CGDM/blob/main/utils.py#L337
    grad_sim1=[]
