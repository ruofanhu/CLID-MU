...
name2sampler = {'RandomSampler': DistributedSampler,'None': None}
...
def split_ssl_data(args, data, targets, num_classes,
    ...
    if args.algorithm=='fullysupervised_lossnet':
        if ulb_num_labels>len(lb_idx):
            ulb_idx = lb_idx.copy()

        else:
            ulb_idx = random.sample(range(len(lb_idx)), ulb_num_labels)
    ...
    print('len:',len(lb_idx),'lendata:',len(data[lb_idx]))
