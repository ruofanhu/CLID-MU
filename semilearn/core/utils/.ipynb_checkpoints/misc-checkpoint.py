...
class TBLog:
    ...
    def update_meta_log(self, tb_meta, it, suffix=None, mode="train"):
        """
        Args
            tb_dict: contains scalar values for updating tensorboard
            it: contains information of iteration (int).
        ...
        if suffix is None:
            suffix = ''
        if self.use_tensorboard:
#             for key, value in tb_meta['lid_40'].items():
#                 self.writer.add_scalar('lid_40/' + key, value, it)
#             for key, value in tb_meta['lid_20'].items():
#                 self.writer.add_scalar('lid_20/' + key, value, it)
            for key, value in tb_meta['lid_80'].items():
                self.writer.add_scalar('lid_80/' + key, value, it)
#             for key, value in tb_meta['weight_metrics'].items():
#                 self.writer.add_embedding(mat=value, global_step=it,tag=suffix+key)                
