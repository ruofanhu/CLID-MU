...
from semilearn.nets import MLP, WNet
import torch
...
from semilearn.core.utils import get_dataset, get_data_loader, get_optimizer, get_cosine_schedule_with_warmup, Bn_Controller,get_step_schedule, manifold_loss_soft_exp_no1_
...
class AlgorithmBase:
    ...
    def __init__(
        ...
        **kwargs):
        ...
        self.weight_dict={}
        self.best_clid_loss, self.best_eval_acc, self.best_it = 1000, 0.0, 0
        ...
        self.wnet = MLP(100,1).to(self.device)
        # build optimizer and scheduler
        self.optimizer, self.scheduler = self.set_optimizer()
        self.optimizer_wnet = torch.optim.SGD(self.wnet.parameters(),self.meta_lr, momentum=0.9, nesterov=True, weight_decay=5e-4) #weight_decay=1e-4 SGD LR =0.001 e-4
        # self.optimizer_wnet = torch.optim.AdamW(self.wnet.parameters(),self.meta_lr, weight_decay=5e-4) #weight_decay=1e-4 SGD LR =0.001 e-4
        # self.wnet_scheduler = get_cosine_schedule_with_warmup(self.optimizer_wnet,
        #                                     1048576,
        #                                     num_warmup_steps=self.args.num_warmup_iter)
        # other arguments specific to the algorfithm
        # self.init(**kwargs)
    ...
    def set_dataset(self):
        ...
        self.args.ulb_edest_len = len(dataset_dict['meta']) if dataset_dict['meta'] is not None else 0
        self.print_fn("unlabeled data number: {}, labeled data number {},meta_evaluation data number".format(self.args.ulb_dest_len, self.args.lb_dest_len,self.args.ulb_edest_len))
        if self.rank == 0 and self.distributed:
            torch.distributed.barrier()
        return dataset_dict
    ...
    def set_data_loader(self):
        ...
        if self.dataset_dict['meta'] is not None:
            loader_dict['meta'] =  get_data_loader(self.args,
                                                   self.dataset_dict['meta'],
                                                   self.args.e_batch_size,
                                                   # make sure data_sampler is None for evaluation
    ...
    def set_optimizer(self):
        ...
        # scheduler = get_step_schedule(optimizer,step_size=20,gamma=0.1)
    ...
    def process_batch(self, input_args=None, **kwargs):
        ...
        for arg, var in kwargs.items():
            ...
            if isinstance(var, dict):
                ...
                # var = { k: v.cuda(self.gpu) if (k!='idx_lb' and k!='idx_ulb') else v for k, v in var.items() }
    ...
    def train(self):
        ...
        # data_eval_iter = iter(self.loader_dict['test'])
        

        # self.optimizer_wnet = torch.optim.AdamW(self.wnet.parameters(),self.meta_lr, weight_decay=5e-4) #weight_decay=1e-4 SGD LR =0.001 e-4

        self.etargets = torch.zeros(self.args.num_labels,self.num_classes).cuda()
        self.weights = torch.zeros(self.args.num_labels).cuda()
...
#             print(next(iter(self.loader_dict['train_ulb'])))
            for data_lb, data_ulb in zip(self.loader_dict['train_lb'],
                ...
                self.tb_dict,self.tb_meta, self.log = self.train_step(**self.process_batch(**data_lb, **data_ulb))
                # self.tb_dict = self.train_step(**self.process_batch(data_eval,**data_lb, **data_ulb))
            ...
            # calculate the estimation targets and the 
            # get_estimated_y_weights(self.loader_dict,self.wnet,self.model,self.weights)
    ...
    def evaluate(self, eval_dest='eval', return_logits=False):
        ...
        with torch.no_grad():
            for data in eval_loader:
                ...
                if logits.shape[1]>1:
                    ...
                    loss = F.binary_cross_entropy(torch.sigmoid(logits).squeeze(), y.type(torch.float),reduction='mean')
                clid_loss = manifold_loss_soft_exp_no1_(feat,logits,self.args.threshold)
                y_true.extend(y.cpu().tolist())
                y_pred.extend(torch.max(logits, dim=-1)[1].cpu().tolist())
                y_logits.append(logits.cpu().numpy())
                # y_probs.append(torch.softmax(logits, dim=-1).cpu().numpy())
                total_loss += loss.item() * num_batch
                total_clid += clid_loss.item() * num_batch
        ...
        eval_dict = {eval_dest+'/loss': total_loss / total_num,eval_dest+'/clid_loss': total_clid / total_num, eval_dest+'/top-1-acc': top1, 
    ...
    def get_save_dict(self):
        ...
        save_dict = {
            ...
            'best_clid_loss': self.best_clid_loss,
    ...
    def load_model(self, load_path):
        ...
        self.best_clid_loss = checkpoint['best_clid_loss']
