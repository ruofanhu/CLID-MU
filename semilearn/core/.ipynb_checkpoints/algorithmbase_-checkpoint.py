...
from semilearn.core.utils import get_dataset, get_data_loader, get_optimizer, get_cosine_schedule_with_warmup, Bn_Controller,get_step_schedule
...
class AlgorithmBase:
    ...
    def set_dataset(self):
        ...
        self.args.ulb_edest_len = len(dataset_dict['meta'])
    ...
    def process_batch(self,*args,**kwargs): # **kwargs
        """
        ...
        input_dict = {}
        evalset_keys = ['idx_e', 'x_e_w','x_e_s', 'y_e_true']

#         evalset_keys = ['idx_e', 'x_e_w', 'x_e_s', 'y_e_true']
#        ['idx_ulb', 'x_ulb_w', 'x_ulb_s', 'y_ulb_true']
        for arg, var in kwargs.items():  #kwargs.items()
            if var is None:
                continue
            ...
            if isinstance(var, dict):
                if var.keys() not in input.keys():
                    
                    var = {k: v.cuda(self.gpu) for k, v in var.items()}
            else:
                var = var.cuda(self.gpu)
            input_dict[arg] = var
        ## process evaluation set for cal_metagradient
        for arg, var in args[0].items():
            var = var.cuda(self.gpu)
            arg_ = evalset_keys.pop(0)
            input_dict[arg_] = var
        return input_dict
...
#             if self.it == 40000 or self.it == 50000:
...
#             print(next(iter(self.loader_dict['train_ulb'])))
            for data_lb, data_ulb in zip(self.loader_dict['train_lb'],
                ...
                # if self.args.renew==0 and self.it==500:
                #     for param in self.model.parameters():
                #         param.requires_grad = False
                #     self.model.renew_layers(0)
                #     self.model.cuda()
                #     self.ema_model = self.set_ema_model()
