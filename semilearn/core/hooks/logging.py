        ...
        if self.every_n_iters(algorithm, algorithm.num_eval_iter):
            if algorithm.args.meta_goal == 'feat_expno1N':
                if not algorithm.distributed or (algorithm.distributed and algorithm.rank % algorithm.ngpus_per_node == 0):
                    algorithm.print_fn(f"{algorithm.it + 1} iteration, USE_EMA: {algorithm.ema_m != 0}, {algorithm.tb_dict}, BEST_EVAL_CLID: {algorithm.best_clid_loss}, at {algorithm.best_it + 1} iters")
            else:
                if not algorithm.distributed or (algorithm.distributed and algorithm.rank % algorithm.ngpus_per_node == 0):
            ...
            if not algorithm.tb_log is None:
                ...
                algorithm.tb_log.update_meta_log(algorithm.tb_meta, algorithm.it)
