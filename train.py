...
def get_config():
    ...
    parser.add_argument('--log', type=yaml.load)
    parser.add_argument('--use_emameta', action='store_true', help='Use tensorboard to plot and save curves, otherwise save the curves locally.')
    ...
    parser.add_argument('--eval_set_size', type=int, default=10000, help='num of data serve as evluation set for cal_meta_gradient')
    parser.add_argument('-bsz', '--batch_size', type=int, default=8)
    parser.add_argument('-e_bsz', '--e_batch_size', type=int, default=64)
    ...
    parser.add_argument('--steplr', type=str2bool, default=False, help='Using StepLR or not')
    ...
    '''
    meta-gradient setting
    '''
    ## args for calculating meta-gradient
    parser.add_argument('--lid_k', type=int, default=10)
    parser.add_argument('--weak_only', type=str2bool, default=True, help='only use weak transformation img to calculate lid')
    parser.add_argument('--lr_inner', type=float, default=3e-2, help='learning rate for inner loop(meta-gradient), default equals to outer loop')
    # parser.add_argument('--meta_updating', type=str2bool, default=True, help='intergrate meta-weight into training or not ')
    parser.add_argument('--meta_goal', type=str, default=True, help='ce or feat or logits or none, none meas no metaupdating, plain training')
    parser.add_argument('--threshold', type=float, default=None, help='threshold for sample selection for meta_loss calculation, range [0,1], None meeans use manifold_loss_soft')
    # parser.add_argument('--relabel', type=str2bool, default=True, help='relabel the labeled set or not')
    parser.add_argument('--mixup_alpha', type=float, default=0, help='alpha for beta distribution in mixup')
    # parser.add_argument('--lid_est', type=str, default='TwoNN',help='estimator for LID, mle or TWONN')
    parser.add_argument('--beta', type=int, default=0, help='weight for the relabeling loss. If set as 0, means no relabeling')

    parser.add_argument('--w_f', type=str, default='min_max', help='clamp with 0 is "zero", min_max normalization is min_max,or to add(weight_net)')
    
    parser.add_argument('--renew', type=int, default=None)
...
def main(args):
    ...
    args.load_path = os.path.join(save_path, 'last_model.pth')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    folder,_ = os.path.split(os.path.realpath(args.c))
    paras_ = vars(args)
    del paras_['log'] 
    ...
    # with open(os.path.join(folder, args.tag_ + '.yaml'), 'w+', encoding='utf-8') as w:
    #     lines = []
...
#     out_name = os.path.join('/home/rhu/r_work/higher_semilearn/analysis/toy_two_ball',args.save_name)
#     res = pm.execute_notebook(
#         '/home/rhu/r_work/higher_semilearn/p.ipynb',
#         '/home/rhu/r_work/higher_semilearn/analysis/toy_two_ball/t.ipynb',
