...
class ResNet(MetaModule):
    ...
    def reinitialize_linear_layer(self):
        # Reinitialize weights and biases of the self.linear layer
        if hasattr(self.linear, 'reset_parameters'):
            self.linear.reset_parameters()
        else:
            # Manual reinitialization if reset_parameters is not defined
            nn.init.kaiming_uniform_(self.linear.weight,a=math.sqrt(5))
            if self.linear.bias is not None:
                fan_in, _ = init._calculate_fan_in_and_fan_out(self.linear.weight)
                bound = 1 / math.sqrt(fan_in) if fan_in > 0 else 0
                init.uniform_(self.linear.bias, -bound, bound)        # Kaiming Uniform initialization for weights
