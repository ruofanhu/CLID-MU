...
class MetaLinear(MetaModule):
    ...
    def reset_parameters(self):
        # Reinitialize weights with Kaiming Uniform Initialization
        fan_in, _ = nn.init._calculate_fan_in_and_fan_out(self.weight)
        bound = math.sqrt(1 / fan_in)
        self.weight = to_var(torch.empty_like(self.weight).uniform_(-bound, bound), requires_grad=True)
        
        # Reinitialize biases with Uniform Initialization
        if self.bias is not None:
            self.bias = to_var(torch.empty_like(self.bias).uniform_(-bound, bound), requires_grad=True)
