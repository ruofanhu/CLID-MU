#!/bin/bash
#SBATCH --job-name=plain		# job name
#SBATCH --mem 40G		# memory size
#SBATCH -n 2 			# number of cpu
#SBATCH -N 1 			# number of node
#SBATCH --gres=gpu:1		# number of gpu
##SBATCH -C V100|A100|A100-80G|A100-mig|T4			# name of gpu that you want to use
#SBATCH -C A100|A100-80G			# name of gpu that you want to use
...
meta_goal=${9:-ce}
warmup=${10:-10}
python Train_cifar_plain.py --noise_mode=${corruption_type} --r=${corruption_prob} --lr=${lr} --meta_lr=${meta_lr} --tau=${tau} --dataset=${dataset} --seed=${seed} --meta_bsz=${meta_bsz} --meta_goal=${meta_goal} --warmup=${warmup}