#!/bin/bash
#SBATCH --job-name=vri_red		# job name
...
python Train_redmini_vri.py --lr=${lr} --meta_lr=${meta_lr} --tau=${tau} --seed=${seed} --meta_bsz=${meta_bsz} --meta_goal=${meta_goal} --warmup=${warmup} --split=${split}