...
def create_model():
    ...
    model = ResNet34(pretrained=False,num_classes=args.num_class)
    model = model.cuda()
    return model
...
stats_log=open(f'/home/rhu/r_work/VRI/VRI_DivideMix/checkpoint/{args.dataset}_{args.r}_{args.noise_mode}_warm_up{args.warmup}_{args.lambda_u}_stats_mix_cos{formatted_time}.txt','w')
test_log=open(f'/home/rhu/r_work/VRI/VRI_DivideMix/checkpoint/{args.dataset}_{args.r}_{args.noise_mode}_warmup{args.warmup}_{args.lambda_u}_acc_mix_cos_{formatted_time}.txt','w')
...
if args.method == 'selfsup':
    ...
    net1 = create_model()
    net2 = create_model()
    vnet1 = vri_mix(576, 1024, 512, args.num_class).cuda()
    vnet2 = vri_mix(576, 1024, 512, args.num_class).cuda()
...
sch_lr_1 = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer1, T_max=args.num_epochs, eta_min=1e-3) 
sch_lr_2 = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer2, T_max=args.num_epochs, eta_min=1e-3) 
...
for epoch in range(args.num_epochs):   
    ...
    lr = args.lr * ((0.1 ** int(epoch >= 120)) * (0.1 ** int(epoch >= 200)))       
