#!/bin/bash
#SBATCH --job-name=vri_a2		# job name
...
model=${8:-vgg}
cos_lr=${9:-False}
python Train_animal_vri_two.py --lr=${lr} --meta_lr=${meta_lr} --tau=${tau} --seed=${seed} --meta_bsz=${meta_bsz} --meta_goal=${meta_goal} --warmup=${warmup} --model=${model} --cos_lr=${cos_lr}