#!/bin/bash
#SBATCH --job-name=wnet_d		# job name
...
meta_goal=${9:-ce}
warmup=${10:-10}
python Train_cifar_wnet.py --noise_mode=${corruption_type} --r=${corruption_prob} --lr=${lr} --meta_lr=${meta_lr} --tau=${tau} --dataset=${dataset} --seed=${seed} --meta_bsz=${meta_bsz} --meta_goal=${meta_goal} --warmup=${warmup}