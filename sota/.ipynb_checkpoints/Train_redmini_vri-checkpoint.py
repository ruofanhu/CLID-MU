...
parser.add_argument('--alpha', default=1.5, type=float, help='parameter for Beta')
...
parser.add_argument('--split', type=str, default='red_noise_nl_0.8')
...
def warmup(net,optimizer,dataloader,args):
    net.train()
    for batch_idx, (inputs, labels, path) in enumerate(dataloader):      
        ...
        #     L+=0.1*clid_loss(feat,outputs,args.tau)
        L.backward()  
        optimizer.step() 
...
def meta_step(net, vnet, optimizer, optimizer_vnet,image, targets, guess_targets,metaloader,args,batch_size,t,warm_up):
        targets=targets.cuda()
        guess_targets=guess_targets.cuda()
...
#####
            ...
            Lx, Lu, lamb = criterion(v_lambda[:batch_size*2] *logits_x, targets[:batch_size*2], v_lambda[batch_size*2:]*logits_u, guess_targets[batch_size*2:],t, warm_up)
...
log.flush()
if '0.8' in args.split:
    args.train_split_file = 'noise_0.8'
elif '0.4' in args.split:
...
for epoch in range(args.num_epochs+args.warmup):   
    ...
    if args.cos_lr:
        ...
        lr = args.lr * ((0.1 ** int(epoch >= 150+args.warmup)) * (0.1 ** int(epoch >= 200+args.warmup)))
    ...
    if epoch<args.warmup:     # warm up
        ...
        print('Warmup Net1')
        warmup(net1,optimizer1,train_loader,args)     
        train_loader = loader.run('warmup')
        print('\nWarmup Net2')
        warmup(net2,optimizer2,train_loader,args)                  
    else:                 
