#!/bin/bash
#SBATCH --job-name=vri_c1		# job name
...
#SBATCH -C A100|A100-80G|H200|H100|L40S			# name of gpu that you want to use
...
python Train_clothing1M_vri.py --lr=${lr} --meta_lr=${meta_lr} --tau=${tau} --seed=${seed} --meta_bsz=${meta_bsz} --meta_goal=${meta_goal} --warmup=${warmup}