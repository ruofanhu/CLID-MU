...
parser.add_argument('--moco_pretrained', default='pretrained/ckpt_clothing_resnet50.pth', type=str)
...
def eval_train(model, loader):
    ...
    with torch.no_grad():
        for batch_idx, (inputs,targets, ids) in enumerate(loader):
            if type(ids)!=list:
                ids = list(ids)
            inputs, targets = inputs.cuda(), targets.cuda()
...
log=open(f'checkpoint/clithin1m_{args.model}_{args.meta_goal}_{args.meta_lr}_{args.tau}_{formatted_time}_{args.warmup}.txt','w')
log.flush()
...
all_loader = loader.run('eval_all')
...
meta_dataset = dataloader.clothing_dataset(args.data_path, transform=transform_train, mode='meta')
...
for epoch in range(args.num_epochs+args.warmup):   
    ...
    if epoch<args.warmup:
        ...
        if args.meta_goal=='ce_sloss':
            ...
            meta_loader_1 = eval_train(net1, all_loader)
