...
from semilearn.algorithms.hooks import PseudoLabelingHook, FixedThresholdingHook, MetaGradientHook,MetaNetHook
...
class UDA_net(AlgorithmBase):
    ...
    def set_hooks(self):
        ...
        self.register_hook(MetaNetHook(), "MetaNetHook")
...
#             tb_meta = self.call_hook("cal_meta_gradient","MetaGradientHook", x_lb,y_lb, x_ulb_w2, y_ulb_true2, 40,'l2', y_true, self.model, self.optimizer)
        ...
        #         logits_x_lb = outs_x_lb['logits']
            ...
            # probs_x_ulb = torch.softmax(logits_x_ulb_s, dim=-1)
            # max_probs, y = torch.max(probs_x_ulb, dim=-1)
            # flag = torch.sum(max_probs.ge(0.4)).item()>0            
        #         with torch.no_grad():
...
#             sup_loss_true = (ce_loss(logits_x_lb, y_true, reduction='none') * sup_mask).mean()
            ...
            # cost_v= ce_loss(logits_x_lb, y_lb, reduction='none')
            ...
            weight_log['w_lid_feat_l2']= w.cpu()/len(w)
            # print(w.cpu())
            # ce_l=ce_loss(logits_x_lb, y_lb, reduction='none')
            # print(torch.sum(ce_l*w).item())
            # ce_l=torch.reshape(ce_l, (len(ce_l), 1))
            if self.args.meta_goal is not None:
            # if self.args.meta_updating and self.it>self.args.s_it:
            # if self.args.meta_updating and flag:
#                 print('\n','meta',self.it, '\n')
                sup_loss = torch.sum(ce_loss(logits_x_lb, y_lb, reduction='none')*w)/len(w)
                sup_loss_true = ce_loss(logits_x_lb, y_true, reduction='mean')
                sup_loss_true_weighted = (ce_loss(logits_x_lb, y_true, reduction='none') *w).sum().item()/len(w)
